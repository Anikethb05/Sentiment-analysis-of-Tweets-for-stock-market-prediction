...
def get_historical_data(symbol, days):
    try:
        ...
        df['middle_band'] = df['Close'].rolling(window=20).mean()  # Explicitly calculate middle band
        df['upper_band'] = df['middle_band'] + (df['20SD'] * 2)   # Element-wise addition
        df['lower_band'] = df['middle_band'] - (df['20SD'] * 2)   # Element-wise subtraction
...
# def train_model(stock):
#     """
#     Enhanced training function that incorporates:
#     1. Technical indicators fetched from Yahoo Finance
#     2. Sentiment scores from tweets
#     3. Integration of snscrape for additional tweet data
#     Without saving any new data to disk
#     """
#     # Step 1: Load existing data
#     df = pd.read_csv('stock_tweets.csv')
#     df[['sentiment_score', 'Negative', 'Neutral', 'Positive']] = df['Tweet'].apply(get_sentiment)
...
#     print(f"Fetching additional tweets for {stock_name} using snscrape...")
#     try:
#         # Set date range for tweet scraping (last 30 days)
#         end_date = datetime.now()
#         start_date = end_date - timedelta(days=30)
        
#         # Format dates for snscrape query
#         start_str = start_date.strftime('%Y-%m-%d')
#         end_str = end_date.strftime('%Y-%m-%d')
        
#         # Construct snscrape command
#         query = f"${stock_name} OR #{stock_name} OR {stock_name} stock lang:en since:{start_str} until:{end_str}"
#         command = ["snscrape", "--jsonl", "--max-results", "100", "twitter-search", query]
        
#         # Execute snscrape command and capture output
#         process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
#         output, error = process.communicate()
...
#                 tweet_data = json.loads(line)
#                 new_tweets.append({
#                     'Date': tweet_data['date'].split('T')[0],  # Extract date part only
#                     'Tweet': tweet_data['content'],
#                     'Stock Name': stock_name
#                 })
...
#             # Combine with existing data (in memory only)
#             df = pd.concat([df, new_df], ignore_index=True)
#             print(f"Added {len(new_tweets)} new tweets from snscrape")
#         else:
#             print("No new tweets found from snscrape")
...
#         # Use the fresh data instead of existing data
#         stock_df = yf_data
#         stock_df['Stock Name'] = stock_name  # Add stock name column to match format
